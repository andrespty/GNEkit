...
`GNE Solver` is built around a small set of reusable abstractions for modeling and solving generalized Nash equilibrium problems.

This page explains the main ideas behind the library so the rest of the documentation feels more natural.
...
- the player name
- the number of decision variables they control
...
For Bayesian problems, the library uses `BayesianPlayer`, which extends the same idea with type-dependent information such as:
...
!!! note "Function Signature Requirement"
...
## Standard And Bayesian Problems
